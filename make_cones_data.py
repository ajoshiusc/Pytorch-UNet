...
data = np.zeros((num_samples_training, 256, 256), dtype=np.float16)
masks = np.zeros((num_samples_training, 256, 256), dtype=np.uint8)
...
for i in tqdm(range(num_samples_training)):
...
for i in tqdm(range(num_samples_valid)):
