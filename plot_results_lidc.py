...
image_edges[edges1>0,0] = 1.0
image_edges[edges2>0,1] = 1.0
image_edges[edges3>0,0] = 1
image_edges[edges3>0,1] = 0
image_edges[edges3>0,2] = 1.0


image_prob = np.zeros((img.shape[0],img.shape[1],3))
image_prob += 0.15*np.float32(qmask1[:,:,None])
image_prob += 0.35*np.float32(qmask2[:,:,None])
image_prob += 0.35*np.float32(qmask3[:,:,None])
...
ax[1].set_title(r'Red=.15, Green=.5, Purple=.85', fontsize=8)
