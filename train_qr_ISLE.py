...
def BCEqr(P, Y, q=0.5):
    L = q * Y * torch.log2(P + 1e-16) + (1.0 - q) * (
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=n_train,
            ...
            for batch in train_loader:
                images = batch[:, :, :, :net.n_channels].permute(
                    (0, 3, 1, 2))  #['image']
                ...
                with torch.cuda.amp.autocast(enabled=amp):
                    ...
                    loss = criterion(
                        masks_pred1[0, 1, ], true_masks[0, ],
                        q=Q1) + criterion(
                            masks_pred2[0, 1, ], true_masks[0, ],
                            q=Q2) + criterion(masks_pred3[0, 1, ],
                                              true_masks[0, ],
                                              q=Q3)  # \
                ...
                if global_step % (n_train // (10 * batch_size)) == 0:
                    ...
                    experiment.log({
                        ...
                        val_score,
                        'images':
                        wandb.Image(images[0, 0].cpu()),
                        'masks': {
                            'true':
                            wandb.Image(true_masks[0].float().cpu()),
                            'pred1':
                            wandb.Image(
                                (torch.softmax(masks_pred1, dim=1)[0, 1] >
                                 0.5).float().cpu()),
                            'pred2':
                            wandb.Image(
                                (torch.softmax(masks_pred2, dim=1)[0, 1] >
                                 0.5).float().cpu()),
                            'pred3':
                            wandb.Image(
                                (torch.softmax(masks_pred3, dim=1)[0, 1] >
                                 0.5).float().cpu()),
                        },
                        'step':
                        global_step,
