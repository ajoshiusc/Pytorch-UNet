...
from util.dice_score import multiclass_dice_coeff
import numpy as np
...
def evaluate_grayscale_QR_4Q(net, dataloader, device):
    net.eval()
    num_val_batches = len(dataloader)
    dice_score = 0
...
def evaluate_grayscale_prob(net, dataloader, device):
    net.eval()
    num_val_batches = len(dataloader)
    dice_score = 0
